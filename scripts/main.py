...
from audio import generate_podcast_audio, generate_podcast_audio_openai
...
def load_config() -> tuple[list, list, list]:
    """Loads topics, journals, and adhoc topics from YAML files."""
...
# ─────────────────────────────────────────────
# Cache helpers (avoid re-fetching on reruns)
# ─────────────────────────────────────────────
...
def run(args) -> None:
    ...
    if not args.no_cache and Path(articles_cache).exists():
        ...
        for adhoc in adhoc_topics:
            if adhoc.get("name") and adhoc.get("query"):
                topic_data = fetch_adhoc_topic(adhoc)
                all_topic_data.append(topic_data)

        save_cache(all_topic_data, articles_cache)

    if args.dry_run:
        logger.info("Dry run complete. Articles fetched and cached.")
        total = sum(
    ...
    # 3. Generate summaries + podcast script (with caching)
    ...
    if args.topics_only:
        logger.info("Topics-only mode: skipping podcast audio.")
        build_dashboard(content, output_dir=output_dir)
        logger.info("Dashboard built. Done.")
        return
